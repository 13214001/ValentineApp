        ...
        filter.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (is_checkbox_checked) {
                    Toast.makeText(TwoFragment.this.getActivity(), "checked", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(TwoFragment.this.getActivity(), "not checked", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        functions.get_users(TwoFragment.this.getActivity(), new Functions.VolleyCallback() {
            ...
            public void onSuccess(final int size) {
                for(int i=0;i<size;i++){
                    if(functions.users[i].equals(current_user) && functions.gender[i].equals("male")){
                        only_from_gender.setText("Roses only from girls:");
                    }

                    else if(functions.users[i].equals(current_user) && functions.gender[i].equals("female")){
                        only_from_gender.setText("Roses only from boys:");
                    }
                    names_leader_board.add(functions.users[i]);
